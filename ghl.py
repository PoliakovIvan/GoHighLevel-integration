...
def search_deal(contact_id, URL, headers):
    response = requests.get(f'{URL}/v1/pipelines/your_pipeline_id/opportunities', headers=headers)
    try:
...
def create_deal(contact_id, email, name, URL, headers):
    ...
    response = requests.post(f'{URL}/v1/pipelines/your_pipeline_id/opportunities/', headers=headers, json=data)
    try:
        result = response.json()
...
def update_deal(deal_id, contact_id, email, name, URL, headers):
    ...
    response = requests.put(f'{URL}/v1/pipelines/your_pipeline_id/opportunities/{deal_id}', headers=headers, json=data)
    try:
        result = response.json()
